<template>
  <div>
    <span v-if="props.type === 'number'">
          <p class="label">{{ props.label }} <label v-if="props.required" class="mandatory-indicator">*</label></p>
          <input v-model="formData[props.variable]" :class="numberValidator.hasError.value ? 'err-input' : 'input form-input'" type="number" v-on:input="capture"
                 placeholder="0" :required="props.required"/>
       <span v-if="numberValidator.hasError.value">
          <validation-error variant="Error" :message="numberValidator.errorMessage.value"/>
       </span>
